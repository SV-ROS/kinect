...
class Navigator():
    ...
    def __init__(self,mapPath,waypointFile,waypointTour):
        ...
        # place rtabmap in localization or mapping mode
        rospy.wait_for_service("/rtabmap/set_mode_localization", timeout=60.0)
        self.rtabmap_mode_localization = rospy.ServiceProxy("/rtabmap/set_mode_localization", Empty)
        self.rtabmap_mode_localization()
...
#         # Spin the robot to wake up rtabmap localization
#         rotate = Twist()
#         rotate.angular.z = 0.4
#         rotate.linear.x = 0.05
#         
#         startup_interval = 20.0
#         tick = 0.2
#         clock = 0.0
#         while clock < startup_interval:
...
#         self.cmd_vel_pub.publish(Twist())
            ...
            if state == 10:  # we finished the goal
                ...
                if goalSts == GoalStatus.ABORTED:
                    ...
                    if sts =="OK":  # MSBM accepted waypoint
                        ...
                        del self.waypointTour[self.tourIdx] # remove this leg from the tour
    ...
    def setNextWaypoint(self):
        ...
        if self.tour_finished:
            return

        # check to see if we are finished the tour, else get next leg
        if len(self.waypointTour) == 0:
            msg = "Finished tour. Stopping the robot."
            rospy.loginfo(msg)
            self.say(msg)
